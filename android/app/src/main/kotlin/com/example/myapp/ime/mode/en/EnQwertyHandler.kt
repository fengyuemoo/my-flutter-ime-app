...
/**
 * Strong-isolated candidate engine for EN-QWERTY.
 */
class EnQwertyCandidateEngine(
    ...
    fun commitCandidateAt(index: Int) {
        if (index !in 0 until currentCandidates.size) {
            val msg = "Candidate index out of range: EN_QWERTY index=$index size=${currentCandidates.size}"
            if (isDebuggableApp()) {
                Log.wtf("EnQwertyCandidateEngine", msg)
    ...
    fun commitCandidate(cand: Candidate) {
        ...
        if (idx < 0) {
            val msg = "Candidate not in current EN_QWERTY list: cand=$cand size=${currentCandidates.size}"
            if (isDebuggableApp()) {
                Log.wtf("EnQwertyCandidateEngine", msg)
                throw AssertionError(msg)
            }
